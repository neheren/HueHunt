Shader "Custom/RevealingShader" {
	Properties {
        ...
        _Strengthmagnitude("Magnitude", Vector) = (0,0,1,0)
	}
	SubShader {
        ...
        float _Strengthmagnitude;

		void surf (Input IN, inout SurfaceOutputStandard o) {
			float3 direction = normalize(_LightPosition - IN.worldPos);

            float distVector = IN.worldPos -  _LightPosition;
            //float dist2Obj =  lerp sqrt (pow(distVector,2) + pow(distVector,2));
            float dist2Obj =  1;

			float scale = dot(direction, _LightDirection);
			float strength = (scale - cos(_LightAngle * (3.14 / 360.0)))/ dist2Obj;
